...
var AffinitySchema = import_koishi14.Schema.object({
  ...
  }).default({ variableName: "contextAffinity", messageWindow: 20 }).description("\u4E0A\u4E0B\u6587\u597D\u611F\u5EA6\u8BBE\u7F6E").collapse(),
...
var RelationshipSchema = import_koishi14.Schema.object({
  ...
  relationships: import_koishi14.Schema.array(
    import_koishi14.Schema.object({
      initialAffinity: import_koishi14.Schema.number().default(null).description("\u521D\u59CB\u957F\u671F\u597D\u611F"),
      userId: import_koishi14.Schema.string().default("").description("\u7528\u6237 ID"),
...
var ScheduleSchema = import_koishi14.Schema.object({
  schedule: import_koishi14.Schema.object({
    ...
    startDelay: import_koishi14.Schema.number().default(1e4).description("\u542F\u52A8\u5EF6\u8FDF\uFF08\u6BEB\u79D2\uFF09\uFF0C\u7B49\u5F85\u5176\u4ED6\u63D2\u4EF6\u52A0\u8F7D\u5B8C\u6210"),
...
function createAffinityStore(ctx, config, log) {
  ...
  const makeId = (platform, userId) => `${platform}:${userId}`;
  ...
  const isBlacklisted = (platform, userId, _channelId) => blacklistSet.has(makeId(platform, userId));
  ...
  const updateRelationshipConfig = (userId, relationName, affinityValue) => {
    ...
    if (existing) {
      ...
      if (affinityValue !== void 0) existing.initialAffinity = affinityValue;
    } else {
  ...
  const save = async (seed, value, inited = true, relation = "", extra) => {
    ...
    const row = {
      ...
      userId,
      nickname,
      affinity: clamp2(longTerm + shortTerm),
...
function createAffinityProvider({ config, cache, store }) {
  return async (_args, _variables, configurable) => {
    ...
    const override = manual && typeof manual.initialAffinity === "number" ? manual.initialAffinity : void 0;
    const record = await store.ensure(
      session,
      (value, lo, hi) => Math.min(hi, Math.max(lo, value)),
      override
    );
    const affinityOverride = record.affinityOverride;
    ...
    const coefficient = record.coefficientState?.coefficient ?? config.affinityDynamics?.coefficient?.base ?? 1;
...
function createRelationshipProvider({ store }) {
  return async (args, _variables, configurable) => {
    ...
    if (typeof affinity === "number") {
      const level2 = store.resolveLevelByAffinity(affinity);
      if (!level2) return "";
      return level2.note ? `${level2.relation}\uFF08${level2.note}\uFF09` : level2.relation;
    }
...
function createDeleteMessageTool({ ctx, toolName }) {
  const logger7 = ctx?.logger?.("chatluna-affinity");
  return new class extends import_tools.StructuredTool {
    name = toolName || "delete_msg";
    description = "Deletes (recalls) a message by message ID or the message currently quoted.";
    schema = import_zod.z.object({
      messageId: import_zod.z.string().optional().describe("Optional: specific message ID to delete. Defaults to the quoted message.")
    });
    async _call(input, _manager, runnable) {
      try {
        const session = getSession(runnable);
        if (!session) return "No session context available.";
        const resolvedMessageId = typeof input.messageId === "string" && input.messageId.trim() || session?.quote?.id || session?.event?.quote?.messageId || session?.event?.message?.id || "";
...
function createAnalysisMiddleware(ctx, config, deps) {
  ...
  const executeAnalysis = async (session, botReply) => {
    ...
    try {
      const manual = store.findManualRelationship(session.platform, session.userId);
      const fallback = manual && typeof manual.initialAffinity === "number" ? manual.initialAffinity : void 0;
      ...
      const storedComposite = Number.isFinite(result.affinityOverride) ? Math.round(result.affinityOverride) : computedComposite;
...
function createScheduleManager(ctx, config, deps) {
  const { getModel, getMessageContent: getMessageContent2, resolvePersonaPreset, renderTableImage, log } = deps;
...
\uFF08\u65E5\u7A0B\u56FE\u7247\u6E32\u67D3\u5931\u8D25\uFF0C\u5DF2\u6539\u4E3A\u6587\u672C\u6A21\u5F0F\uFF09`;
  ...
  const start = () => {
    ...
    if (cachedSchedule && cachedDate === dateStr) {
      ...
      const startDelay = scheduleConfig.startDelay ?? 1e4;
...
function apply(ctx, config) {
  ...
  const scheduleManager = createScheduleManager(ctx, config, { getModel, getMessageContent, resolvePersonaPreset, renderTableImage, log });
  ...
  ctx.on("ready", async () => {
    ...
    if (config.enableDeleteMessageTool) {
      ...
      plugin.registerTool(toolName, { selector: () => true, authorization: (session) => session?.platform === "onebot", createTool: () => createDeleteMessageTool({ ctx, toolName }) });
    }
  ...
  ctx.command("affinity.rank [limit:number] [platform:string] [image]", "\u67E5\u770B\u5F53\u524D\u597D\u611F\u5EA6\u6392\u884C", { authority: 1 }).alias("\u597D\u611F\u5EA6\u6392\u884C").action(async ({ session }, limitArg, platformArg, imageArg) => {
    ...
    const lines = await Promise.all(scopedRows.map(async (row) => {
      ...
      return { name: name2, relation: row.relation || "\u2014\u2014", affinity: row.affinity };
    }));
  ...
  ctx.command("affinity.inspect [targetUserId:string] [platform:string]", "\u67E5\u770B\u6307\u5B9A\u7528\u6237\u7684\u597D\u611F\u5EA6\u8BE6\u60C5", { authority: 1 }).alias("\u597D\u611F\u5EA6\u8BE6\u60C5").action(async ({ session }, targetUserArg, platformArg) => {
    const platform = platformArg || session?.platform || "";
  ...
  ctx.command("affinity.tempBlacklist [limit:number] [platform:string]", "\u67E5\u770B\u4E34\u65F6\u9ED1\u540D\u5355\u5217\u8868", { authority: 2 }).alias("\u4E34\u65F6\u9ED1\u540D\u5355").action(async ({ session }, limitArg, platformArg) => {
    const parsedLimit = Number(limitArg);
    const limit = Math.max(1, Math.min(Number.isFinite(parsedLimit) ? parsedLimit : config.blacklistDefaultLimit, 100));
  ...
  ctx.command("affinity.groupList", "\u663E\u793A\u673A\u5668\u4EBA\u5DF2\u52A0\u5165\u7684\u7FA4\u804A", { authority: 2 }).alias("\u7FA4\u804A\u5217\u8868").action(async ({ session }) => {
