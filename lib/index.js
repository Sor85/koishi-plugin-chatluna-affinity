...
__export(index_exports, {
  ...
  registerTempBlacklistCommand: () => registerTempBlacklistCommand,
...
var AffinityDynamicsSchema = import_koishi.Schema.object({
  shortTerm: import_koishi.Schema.object({
    ...
    demoteThreshold: import_koishi.Schema.number().default(-15).description("\u77ED\u671F\u597D\u611F\u4F4E\u4E8E\u8BE5\u503C\u65F6\u964D\u4F4E\u957F\u671F\u597D\u611F"),
    longTermPromoteStep: import_koishi.Schema.number().default(3).min(1).description("\u6BCF\u6B21\u589E\u52A0\u957F\u671F\u597D\u611F\u7684\u5E45\u5EA6"),
    longTermDemoteStep: import_koishi.Schema.number().default(3).min(1).description("\u6BCF\u6B21\u51CF\u5C11\u957F\u671F\u597D\u611F\u7684\u5E45\u5EA6")
  }).default({
    promoteThreshold: 15,
    demoteThreshold: -15,
...
async function fetchGroupList(session, log) {
  try {
    ...
    if (typeof internal._request === "function") {
      const response = await internal._request("get_group_list", {});
      result = response;
    } else if (typeof internal.getGroupList === "function") {
      result = await internal.getGroupList();
    }
    return result;
  } catch (error) {
...
function createGroupInfoProvider(deps) {
  const { config, log, fetchGroupList: fetchGroupList2 } = deps;
  return async (_args, _variables, configurable) => {
    const session = configurable?.session;
...
function registerBlacklistCommand(deps) {
  const { ctx, config, renders, permanentBlacklist, resolveGroupId: resolveGroupId2, stripAtPrefix: stripAtPrefix2 } = deps;
  ctx.command(
    "affinity.blacklist [limit:number] [platform:string] [image]",
    "\u67E5\u770B\u81EA\u52A8\u9ED1\u540D\u5355\u5217\u8868",
    { authority: 2 }
  ).alias("\u81EA\u52A8\u9ED1\u540D\u5355").action(async ({ session }, limitArg, platformArg, imageArg) => {
    const parsedLimit = Number(limitArg);
    const limit = Math.max(
      1,
...
function registerBlockCommand(deps) {
  const { ctx, cache, permanentBlacklist, resolveUserIdentity: resolveUserIdentity2, resolveGroupId: resolveGroupId2, stripAtPrefix: stripAtPrefix2 } = deps;
  ctx.command(
...
function registerTempBlacklistCommand(deps) {
  const { ctx, config, renders, temporaryBlacklist, stripAtPrefix: stripAtPrefix2 } = deps;
  ctx.command(
    "affinity.tempBlacklist [limit:number] [platform:string] [image]",
    "\u67E5\u770B\u4E34\u65F6\u9ED1\u540D\u5355\u5217\u8868",
    { authority: 2 }
  ).alias("\u4E34\u65F6\u9ED1\u540D\u5355").action(async ({ session }, limitArg, platformArg, imageArg) => {
    const parsedLimit = Number(limitArg);
    const limit = Math.max(
      1,
      Math.min(Number.isFinite(parsedLimit) ? parsedLimit : config.blacklistDefaultLimit, 100)
...
function registerScheduleCommand(deps) {
  ...
  ctx.command("affinity.schedule", "\u67E5\u770B\u4ECA\u65E5\u65E5\u7A0B", { authority: 2 }).alias("\u4ECA\u65E5\u65E5\u7A0B").action(async ({ session }) => {
    ...
    if (scheduleConfig.renderAsImage) {
      ...
      if (buffer) return import_koishi27.h.image(buffer, "image/png");
      return `${schedule.text || "\u6682\u65E0\u4ECA\u65E5\u65E5\u7A0B\u3002"}
\uFF08\u65E5\u7A0B\u56FE\u7247\u6E32\u67D3\u5931\u8D25\uFF0C\u5DF2\u6539\u4E3A\u6587\u672C\u6A21\u5F0F\uFF09`;
    }
    return schedule.text || "\u6682\u65E0\u4ECA\u65E5\u65E5\u7A0B\u3002";
