...
function createAffinityProvider({ config, cache, store }) {
  return async (_args, _variables, configurable) => {
    ...
    if (typeof affinityOverride === "number") {
      const result2 = hasManualOverride ? affinityOverride : store.clamp(affinityOverride);
      cache.set(session.platform, session.userId, result2);
      return result2;
    }
...
function createAnalysisMiddleware(ctx, config, deps) {
  ...
  const executeAnalysis = async (session, botReply) => {
    ...
    try {
      ...
      const hasManualOverride = fallback !== void 0;
