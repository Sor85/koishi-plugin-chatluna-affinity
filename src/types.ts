...
export interface ActionStats { total: number; counts: ActionCounts; entries: ActionEntry[] }
export interface ActionCounts { increase: number; decrease: number; hold: number }
...
export interface CombinedState { affinity: number; longTermAffinity: number; shortTermAffinity: number }
export interface InitialRange { low: number; high: number; min: number; max: number }
...
export interface GroupInfoConfig { variableName: string; includeMemberCount: boolean; includeCreateTime: boolean }
export interface RandomConfig { variableName: string; min?: number; max?: number }
...
export interface Config {
  ...
  otherVariables?: { userInfo?: UserInfoConfig; botInfo?: BotInfoConfig; groupInfo?: GroupInfoConfig; random?: RandomConfig }
...
export interface SessionSeed { platform?: string; userId?: string; selfId?: string; nickname?: string; authorNickname?: string; session?: Session }
...
export interface AffinityCache { get: (platform: string, userId: string) => number | null; set: (platform: string, userId: string, value: number) => void; clear: (platform: string, userId: string) => void; clearAll?: () => void }

export interface ScheduleEntry { start: string; end: string; startMinutes: number; endMinutes: number; summary: string }
export interface Schedule { source: string; date: string; title: string; description: string; entries: ScheduleEntry[]; text: string }
...
export interface InMemoryTemporaryEntry { expiresAt: number; nickname: string }
export interface TemporaryBlacklistManager { isBlocked: (platform: string, userId: string) => InMemoryTemporaryEntry | null; activate: (platform: string, userId: string, nickname: string, now: Date) => { activated: boolean; entry: InMemoryTemporaryEntry | null }; clear: (platform: string, userId: string) => void }
export interface ShortTermOptions { enabled: boolean; windowHours: number; windowMs: number; decreaseThreshold: number; durationHours: number; durationMs: number; penalty: number }
export interface RoleTranslation { role: string; matched: boolean; raw: unknown }
export interface RoleMapping { direct: Record<string, string>; keywords: Record<string, string[]>; numeric: Record<string, string> }

export type LogLevel = 'debug' | 'info' | 'warn' | 'error'
export type LogFn = (level: LogLevel, message: string, detail?: unknown) => void
export interface ChatLunaPlugin { registerTool: (name: string, options: ToolRegistration) => void }
export interface ToolRegistration { selector: () => boolean; createTool: () => unknown; authorization?: (session: Session) => boolean }

export interface ResolvedShortTermConfig { promoteThreshold: number; demoteThreshold: number; longTermPromoteStep: number; longTermDemoteStep: number }
export interface ResolvedActionWindowConfig { windowHours: number; windowMs: number; increaseBonus: number; decreaseBonus: number; bonusChatThreshold: number; allowBonusOverflow: boolean; maxEntries: number }
export interface ResolvedCoefficientConfig { base: number; maxDrop: number; maxBoost: number; decayPerDay: number; boostPerDay: number; min: number; max: number }
export interface CoefficientResult { coefficient: number; decayPenalty: number; streakBoost: number; inactivityDays: number }
export interface SummarizedActions { entries: ActionEntry[]; counts: ActionCounts; total: number }

export type MemberInfoField = 'nickname' | 'userId' | 'role' | 'level' | 'title' | 'gender' | 'age' | 'area' | 'joinTime' | 'lastSentTime'
export interface MemberInfo {
  card?: string; remark?: string; displayName?: string; nick?: string; nickname?: string; name?: string
  user?: { nickname?: string; name?: string }
  level?: string | number; levelName?: string; level_name?: string; level_info?: { current_level?: string | number; level?: string | number }
  title?: string; specialTitle?: string; special_title?: string
  sex?: string; gender?: string; age?: number
  area?: string; region?: string; location?: string
  join_time?: number | string; joined_at?: number | string; joinTime?: number | string; joinedAt?: number | string; joinTimestamp?: number | string
  last_sent_time?: number | string; lastSentTime?: number | string; lastSpeakTimestamp?: number | string
  role?: string; roleName?: string; permission?: string; permissions?: string | string[]; identity?: string; type?: string; status?: string; roles?: string[]
  userId?: string; id?: string; qq?: string; uid?: string; user_id?: string
}

export interface RenderMemberInfoOptions { fallbackNames?: string[]; defaultItems?: MemberInfoField[]; logUnknown?: boolean; log?: LogFn }
export interface GroupInfo { group_id?: string | number; groupId?: string | number; id?: string | number; group_name?: string; groupName?: string; name?: string; member_count?: number; memberCount?: number; max_member_count?: number; create_time?: number | string; createTime?: number | string }
export interface NormalizedTime { minutes: number; label: string; raw: string }
export interface AnalysisMiddlewareResult { middleware: (session: Session, next: () => Promise<void>) => Promise<void>; addBotReply: (session: Session, botReply: string) => void; cancelScheduledAnalysis: (session: Session) => void }
export interface PendingAnalysis { session: Session; timestamp: number; botReplies: string[]; timer: ReturnType<typeof setTimeout> | null }
