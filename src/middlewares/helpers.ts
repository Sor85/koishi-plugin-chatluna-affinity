...
export function parseModelResponse(
  ...
  const { maxIncreaseLimit, maxDecreaseLimit, log, debugEnabled, nickname, session } = options
  ...
  if (match) {
    try {
      ...
      action = typeof parsed.action === 'string' ? parsed.action.toLowerCase() : ''
  ...
  const limitedDelta = delta >= 0
    ? Math.min(delta, maxIncreaseLimit)
    : Math.max(delta, -maxDecreaseLimit)

  return { delta: limitedDelta, action, reason }
}
...
export function applyAffinityChanges(
  ...
  result: AffinityState,
  _config: Config,
  data: ApplyAffinityChangesData
): AffinityChangesResult {
  const { actionWindow, shortTermRules, store } = data
  const eligibleChatCount = data.summarizedActions.total
  ...
  const positiveCapacity = allowOverflow
    ? Math.max(0, positiveBonus)
    : Math.max(0, Math.min(data.maxIncreaseLimit - delta, positiveBonus))
  const negativeCapacity = allowOverflow
    ? Math.max(0, negativeBonus)
    : Math.max(0, Math.min(data.maxDecreaseLimit - Math.abs(delta), negativeBonus))

  const extraFromHistory = delta > 0
    ? positiveCapacity
    : delta < 0
      ? -negativeCapacity
      : 0

  const appliedDelta = delta + extraFromHistory
  const actionType: 'increase' | 'decrease' | 'hold' = delta > 0 ? 'increase' : delta < 0 ? 'decrease' : 'hold'

  const baselineShortTerm = Math.round(result.shortTermAffinity ?? 0)
  const baselineLongTerm = Math.round(result.longTermAffinity ?? result.affinity ?? 0)

  let workingShortTerm = baselineShortTerm + appliedDelta
  let longTermTarget = baselineLongTerm
  let longTermShift = 0
  ...
  return {
    ...
    longTermChanged,
    extraFromHistory,
    positiveBonus: extraFromHistory > 0 ? extraFromHistory : 0,
    negativeBonus: extraFromHistory < 0 ? Math.abs(extraFromHistory) : 0
  }
}
