...
const normalizeAction = (action: unknown): ActionType => {
...
export function resolveActionWindowConfig(config: Config): ResolvedActionWindowConfig {
  ...
  return { windowHours, windowMs: windowHours * 3600 * 1000, increaseBonus, decreaseBonus, bonusChatThreshold, allowBonusOverflow, maxEntries }
}
...
export function summarizeActionEntries(rawEntries: ActionEntry[] | undefined, windowMs: number, nowMs: number): SummarizedActions {
  const fallback: SummarizedActions = { entries: [], counts: { increase: 0, decrease: 0, hold: 0 }, total: 0 }
...
export function appendActionEntry(entries: ActionEntry[] | undefined, action: ActionType | string, nowMs: number, maxEntries: number): ActionEntry[] {
...
export function computeShortTermReset(): number { return 0 }

const dayNumber = (date: Date): number => Math.floor(date.getTime() / 86400000)

export function computeDailyStreak(previousStreak: number | undefined, lastInteractionAt: Date | null | undefined, now: Date): number {
...
export function computeCoefficientValue(coefConfig: ResolvedCoefficientConfig, streak: number, lastInteractionAt: Date | null | undefined, now: Date, todayIncreaseCount: number = 0, todayDecreaseCount: number = 0): CoefficientResult {
